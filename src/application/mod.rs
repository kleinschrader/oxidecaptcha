use crate::{config::Config, routes::{delete_challange, get_challange}, state::State, storage::{self, StorageProvider}};
...
impl Application {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        let timeout_middleware = axum::middleware::from_fn(
            crate::middleware::timeout_middleware,
        );
        ...
        let site_challenge_router = axum::Router::new()
            .route("/site/:siteId/challenge/:challengeId", delete(delete_challange))
    ...
    fn parse_config() -> Result<Config> {
        let config = std::fs::read_to_string("config.json")
            .context("Unable to open config.json")?;
