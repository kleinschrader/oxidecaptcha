...
impl Storage for StorageProvider {
    ...
    async fn store_challenge(&self, site: &crate::site::Site, challenge: &Challenge<'static, ()>) -> Result<(), super::StorageError> {
        match self {
            StorageProvider::Memory(memory_storage) => memory_storage.store_challenge(site, challenge).await
        }
    }
    
    async fn delete_challenge(&self, site: &crate::site::Site, challenge: &Challenge<'static, ()>) -> Result<(), super::StorageError> {
        match self {
            StorageProvider::Memory(memory_storage) => memory_storage.delete_challenge(site, challenge).await
        }
    }
}
        ...
        match config {
            crate::config::StorageTypeConfig::Memory(in_memory_config) => {
                ...
                StorageProvider::Memory(
                    MemoryStorage::new(in_memory_config.get_sites(), duration, house_config.batch_size)
                )
            },
        }
    }
